...
export const Person = {
  ...
  addRelationship(personAId, personBId, type) {
    ...
    db.prepare('INSERT INTO relationships (id, person_a_id, person_b_id, type) VALUES (?, ?, ?, ?)')
      .run(id, personAId, personBId, type);
    return { id, personAId, personBId, type };
  },
...
export const Tag = {
  getAll() {
    const db = getDB();
    return db.prepare(`SELECT t.*, (SELECT COUNT(*) FROM story_tags WHERE tag_id = t.id) AS count
      FROM tags t ORDER BY t.name ASC`).all();
  },

  getByName(name) {
    const db = getDB();
    return db.prepare('SELECT * FROM tags WHERE name = ?').get(name.toLowerCase().trim());
  }
};
